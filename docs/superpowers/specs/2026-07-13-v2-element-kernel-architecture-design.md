...
“零普通依赖”以发布 tarball、根 package.json、源码 import 和真实消费安装结果为准。开发工具或 website workspace 的传递依赖仍可能让 package-lock.json 出现同名包；这不属于 engine 运行依赖，也不能成为源码继续直接引用它们的理由。
...
两个宏观步骤分别使用独立实施计划：书面规格终审后只制定并执行第一步计划；第一步 API 冻结后再制定第二步文档计划。每个内部阶段都必须有验证证据、审查检查点和提交边界；阶段之间保持代码测试可验证，不在外部发布半迁移状态。若某阶段需要改变本文公共契约，必须先提交补充设计并重新获得批准。
