...
- 不保证 nativeStyle 可序列化、可执行属性动画或跨 OpenLayers 主版本兼容。
...
- Core 不导入任何 OpenLayers 类型或运行时代码。
...
- Earth 公共构造函数可以作为高级入口保留，但文档优先展示 useEarth。
...
Element 是唯一业务对象，ElementState 是可快照、可事务更新的规范状态。它至少包含：

- id：单元素身份。
- type：图形类型。
- geometry：与 OpenLayers 无关的图形状态。
- style：StyleSpec 或显式 nativeStyle。
- data：业务数据。
- module：业务模块分组。
- layerId：渲染图层分组。
- visible：业务可见状态。
...
Element.olFeature 作为只读高级逃生口保留，用于命中检测、对接 OL 插件或临时读取原生对象。通过该入口直接修改原生对象不建立业务一致性保证；持久修改必须提交到 ElementStore。
...
对外使用判别式 LayerSpec，而不是大量构造参数和几何专用类。常用底图能力通过少量内置 preset 提供，同时允许高级用户传入公开的 OL Source 或原生 Layer。默认配置、文档和示例不得包含私有 token、账号或内网地址。
...
DOM Overlay 不进入 ElementStore，但支持 id、module 和 data 查询及批量清理。现有 offset、positioning、stopEvent、insertFirst、autoPan、className 等能力必须保留。
...
每个 Earth 只安装一套视口输入监听，由 InputRouter 和 EventService 分发。订阅统一返回幂等 disposer。
...
浏览器原生 contextmenu 只在对应 Earth viewport 内始终 preventDefault：
...
包中删除 ol-wind 和 wind-core。所有 OL 导入使用官方公开 ESM 模块路径，不依赖 renderer 私有类型或带下划线字段。生成声明不得暴露内部 Adapter 类型。

## 17. 实施分解与 OL 10 升级顺序

本设计必须在独立 2.0 分支内分阶段实施。阶段之间保持测试可验证，不在外部发布半迁移状态。
...
- 删除 Wind 及依赖。
- 升级 OpenLayers 10.9、TypeScript 和模块解析配置。
...
能力矩阵来源必须同时覆盖公共导出、源码、现有测试、网站文档和可运行示例。除 Wind 外，每一项必须记录：
...
所有公开 API、行为、参数和示例必须在实现所在阶段同步更新 website，不把文档留到最后补写。
...
| OverlayLayer、Descriptor | Earth.overlays 及 Descriptor 复合能力 |
...
- OpenLayers 10.9、ESM 包、类型声明和真实消费者验证通过。
