...
- `radar-scan.gradient` 与 `path-travel.gradient` 使用同一格式、校验和线性 RGBA 插值：至少两个 `[offset, Color]` 色标，offset 是 `[0, 1]` 内严格递增的有限数；色标范围之外沿用最近端点颜色。
...
- 实现 radar-scan、center-spread、雷达渐变尾迹、Circle 跨界连续角区间、Sector 裁剪、弧线采样缓存、固定 slot 和 world wrap。
...
1. 每个 Spec 的默认值、边界、未知字段、非法对象、输入不变性和错误类型；radar-scan 还覆盖默认绿色、gradient 色标格式及 color/gradient 互斥。
...
9. Circle 在不同纬度和 View 投影下的米制半径转换；radar-scan 渐变 offset 端点、RGBA 插值与最终 alpha；Circle 顺逆时针跨 `0/2π`（含正北起扫位置）的连续尾迹；Sector 起止边界、跨界方向和 beam 裁剪。
...
- fade-out retain、DoubleArrow grow、radar-scan 默认绿色与渐变 alpha、Circle `0/2π` 无接缝、Sector radar 裁剪和 interaction pause-and-suppress 具有明确自动化证据。
