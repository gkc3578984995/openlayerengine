...
- 契约修订：2026-07-21，`radar-scan` 增加可选 `scanMode?: 'one-way' | 'round-trip'`，默认保持 `one-way`；`direction` 固定表示首程方向，`periodMs` 固定表示完整模式周期；`round-trip` 使用按 elapsed 解析式回看的 recent-history 尾迹，在折返点采用 newest-wins，保持固定 10 个 slot 且不记录真实渲染帧历史
- 契约修订：2026-07-21，`radar-scan` 与 `center-spread` 的 `color` 纯色模式对全部 active overlay slot 使用恒定 `opacity`，不再叠加尾迹年龄或扩散进度 alpha 衰减；`gradient` 模式继续保留既有 `slotIntensity` 与 `center-spread` 生命周期渐隐公式
- 契约修订：2026-07-21，LayerRenderPass 在同一 postrender 批次复用 OpenLayers 立即渲染器时，逐 primitive 只保存并恢复 `globalAlpha` 数值，不再对完整 Canvas context 执行 `save()` / `restore()`，避免 Canvas 样式状态回滚后与立即渲染器的公开绘制状态缓存失配
...
export interface RadarScanAnimationSpec {
  ...
  readonly scanMode?: 'one-way' | 'round-trip';
  readonly color?: Color;
...
| `radar-scan`    | `periodMs: 2000`、`direction: 'clockwise'`、`scanMode: 'one-way'`、`color: '#00e676'`、`gradient: undefined`、`opacity: 0.35`、`beamWidthDeg: 45`、`repeat: true` | `repeat: false` 时经过一个完整模式周期后移除         |
...
- `radar-scan.scanMode` 只接受 `'one-way'` 或 `'round-trip'`，默认值为 `'one-way'`；`direction` 只接受 `'clockwise'` 或 `'counterclockwise'`，在两种模式下都表示首程方向，不得根据 `scanMode`、Sector 边界顺序或当前 View rotation 隐式改写。
- `radar-scan.periodMs` 始终表示当前 `scanMode` 的完整周期：`one-way` 在该时长内完成一次单程扫描，`round-trip` 在前半周期完成首程、后半周期沿相反方向返回；不得在 normalize 或 Adapter 中把同一字段解释为 `round-trip` 的单程时长。
...
LayerRenderPass 在立即绘制单个展示替身及其 overlay 时，必须在现有 Canvas alpha 基础上相乘，并保证异常路径恢复进入该 primitive 前的 `globalAlpha`：

```ts
const previousGlobalAlpha = context.globalAlpha;
try {
  context.globalAlpha = previousGlobalAlpha * effectiveOpacity;
  drawPresentation();
} finally {
...
同一 postrender 批次会复用一个 OpenLayers 立即渲染器。禁止在相邻 primitive 之间逐次 `save()` / `restore()` 整个 Canvas context：`restore()` 会回滚 fill、stroke 等真实 Canvas 状态，而立即渲染器仍保留上一笔样式缓存，后续同色 slot 可能因此沿用 Canvas 默认颜色。不得读取或重置 OpenLayers 私有缓存；若未来确需恢复 alpha 之外的 Canvas 状态，应在整个批次的外边界统一处理并补充验证。
...
- `direction` 表示首程在最终屏幕上的视觉旋转方向；clockwise 和 counterclockwise 的含义不受 View rotation 影响。Circle 的首程从 View 坐标正 Y 方向开始；Sector 的首程从与 direction 对应的边界开始。
- `scanMode: 'one-way'` 保持既有单向扫描：一个完整周期从首程起点覆盖全部 sweep 并到达终点。`scanMode: 'round-trip'` 在同一周期内先按 direction 从起点覆盖全部 sweep 到达终点，再沿相反方向返回起点；不得在返回时交换或重建 radial-frame 的业务边界。
- 令 `P = periodMs`，当前完整模式周期进度为 `c = repeat ? positiveModulo(elapsedMs / P, 1) : clamp(elapsedMs / P, 0, 1)`。`one-way` 的首程归一化位置为 `u = c`；`round-trip` 为 `u = c <= 0.5 ? 2 * c : 2 * (1 - c)`，因此精确满足 elapsed `0` 在起点、`P / 2` 在终点、`P` 回到起点。`repeat: false` 在 elapsed 到达 `P` 时完成并移除，不把 round-trip 的返回程登记为第二个周期；`repeat: true` 继续以 `P` 为周期。
- beamWidthDeg 是尾迹角宽，Circle 最大为 360°，Sector 最大为自身 sweep。
- `color` 提供纯色尾迹；`gradient` 提供沿尾迹的渐变色，两者互斥。gradient offset `0` 表示尾迹最旧端，`1` 表示扫描前沿；各颜色通道和 Color 自身 alpha 均使用与 `path-travel` 相同的线性插值规则。
- 尾迹固定使用 10 个 age slot。令 `i = 0` 表示扫描前沿的最新 slot，`i = 9` 表示最旧 slot，则 `gradientOffset = 1 - i / 9`，最新 slot 强度为 1，第 `i` 个旧 slot 的 `slotIntensity = 1 - i / 10`。gradient 模式令 `resolvedColor` 为该 offset 的插值颜色，最终 alpha 固定为 `alpha(resolvedColor) * opacity * slotIntensity * targetOpacity`；color 纯色模式的全部 active slot 最终 alpha 固定为 `alpha(color) * opacity * targetOpacity`，与 `i` 无关，不叠加 `slotIntensity`。恒定只约束 active slot，边界裁剪产生的 inactive 区域仍不绘制。
- `one-way` 保持既有空间尾迹算法：把实际 beam 宽度均分为 10 个连续扇形 slot，slot 随当前首程位置推进；repeat Circle 在完整 `2π` 连续角域内跨周期保留完整尾迹，Sector 与 non-repeat Circle 在起始边界按已扫过范围裁剪。
- `round-trip` 使用解析式 recent-history，而不是在折返点清空尾迹或把尾迹瞬时翻到扫描前沿另一侧。令单程时长 `L = P / 2`、实际 beam 弧度 `B = min(beamWidthDeg * π / 180, sweepAngleRad)`、完整尾迹时间窗 `H = L * B / sweepAngleRad`、每个 slot 的年龄宽度 `Δ = H / 10`。elapsed 为 `t` 时，第 `i` 个 slot 取历史区间 `[t - (i + 1) * Δ, t - i * Δ]` 与 `[0, t]` 的交集，并用同一 round-trip 三角时间线解析该区间实际扫过的角度像；空交集 inactive。禁止用已渲染帧、实际帧间隔或坐标队列近似这段历史。
- 因为 `B <= sweepAngleRad`，所以 `Δ <= L / 10`，单个 age slot 的历史区间至多跨越一个折返点，其角度像始终是一个连续区间且宽度不超过 `B / 10`。跨越折返点时必须把边界极值纳入区间，不能只连接历史区间的两个端点；每个 age slot 仍只写一个预分配扇形 geometry，不得为入程、返程或折返点临时增加 slot。
- round-trip 在折返点附近会让不同年龄 slot 的角度像重叠，重叠区固定采用 newest-wins：较小 `i` 的更新 slot 独占该角度，较旧 slot 在写入 geometry 前扣除已由更新 slot 覆盖的区间，不通过重复 alpha 叠加改变上述 color / gradient opacity 公式。该归属必须在 Runtime 输出 geometry 前确定，Adapter 不得用偶然的绘制顺序替代或选择相反的覆盖语义。
- Circle 使用不截断的连续角区间生成尾迹。扫描前沿或尾迹跨过任意 `0/2π` 表示边界时，允许角度超出标准区间并由三角函数自然闭合，或等价拆分为两段完整绘制；不得先把首尾角分别归一化后按大小裁剪，从而在正北等跨界位置留下空白扇区。Sector 必须在同一连续角度域内与自身起止边界求交，跨界处理不得取消 Sector 裁剪或越过边界射线。
- 尾迹固定使用 10 个扇形 slot，内部协议硬上限仍为 16；round-trip 不得为两个方向分别预分配一套 slot，slot、坐标容量和对象数量都不随播放时长或往复次数增长。
- repeat 为 false 时经过一个完整模式周期 `P` 后移除；one-way 的最终路径为起点到终点，round-trip 的最终路径为起点到终点再返回起点。

### 8.6 center-spread

- 要求 radial-frame 和 structured-presentation，写入 overlay。
- Circle 和 Sector 都绘制沿半径方向由内向外扩散的波纹带，视觉语义是 radar-scan 角度尾迹的径向对应物。Circle 的每段填充是完整、无 `0/2π` 接缝的环带；Sector 的每段填充是严格裁剪在自身 sweep 内的环形扇面，不得越过两条边界射线。前沿 stroke 遵循同一完整环或裁剪弧语义。
- `color` 提供纯色波纹带；`gradient` 提供沿半径方向的尾迹渐变，两者互斥。gradient offset `0` 表示内侧最旧尾迹，`1` 表示外侧波纹前沿；各颜色通道和 Color 自身 alpha 均使用与 `radar-scan`、`path-travel` 相同的线性插值规则。
- `periodMs` 表示单个环从中心传播到外半径的寿命。发射间隔 `interval = periodMs / ringCount`，第 `r` 个环首次发射时间为 `r * interval`；repeat 为 true 时该环此后每隔 periodMs 再次发射。
- 有效环进度为 `p = (elapsedMs - emittedAt) / periodMs`；只在 `0 <= p < 1` 时绘制，前沿半径为 `radialRadius * p`。令 `w = trailLength / 4`，每环固定预分配四个填充 slot；第 `i` 个 slot 中 `i = 0` 表示最靠外、最新的尾迹，`i = 3` 表示最靠内、最旧的尾迹，其归一化外、内半径分别为 `clamp(p - i * w, 0, 1)` 和 `clamp(p - (i + 1) * w, 0, 1)`。内外半径相等时该 slot inactive。
- 第 `i` 个填充 slot 使用 `gradientOffset = 1 - i / 3` 和 `slotIntensity = 1 - i / 4`。gradient 模式令 `resolvedColor` 为该 offset 的插值颜色，填充最终 alpha 固定为 `alpha(resolvedColor) * opacity * slotIntensity * (1 - p) * targetOpacity`；color 纯色模式的全部可见填充 slot 固定使用 `alpha(color) * opacity * targetOpacity`，不得叠加 `slotIntensity` 或 `(1 - p)`，使同一纯色波纹带在整个传播生命周期内保持均匀透明度。
- 每环另有一个独立的前沿 stroke slot，颜色使用 gradient offset `1` 的插值颜色，未配置 gradient 时使用 color。gradient 模式的最终 alpha 固定为 `alpha(frontColor) * opacity * (1 - p) * targetOpacity`；color 纯色模式固定为 `alpha(color) * opacity * targetOpacity`。线宽为 `strokeWidth`。`trailLength: 0` 时四个填充 slot 保持预分配但全部 inactive，只绘制该前沿 stroke，从而退化为旧式完整线环或裁剪弧；不得因退化改变 slot 拓扑。
- 纯色恒定只表示 alpha 不随 `i` 或 `p` 改变；Color 自身 alpha、Spec `opacity` 和其他效果合成后的 `targetOpacity` 仍参与最终 alpha，`p = 1` 时 slot 按生命周期 inactive。模式以字段为准，即使 gradient 的全部色标使用同一颜色也仍执行 gradient 衰减，不等同于 color 纯色模式。
- `ringCount` 硬上限为 5；每环固定四个填充 slot 加一个前沿 stroke slot，因此单个 Runtime 默认 15 个、最多 25 个 slot。环带和前沿弧的采样数按屏幕误差和 resolution 桶缓存并设置硬上限，slot 数、坐标容量和对象数量不得随播放时长增长。
- repeat 为 false 时每个环只发射一次，完成时间固定为 `periodMs + (ringCount - 1) * interval`；最后一环完成后移除全部 5 × ringCount 个 slot。repeat 为 true、pause/resume、hide/show、stop、replace、remove 和 Earth.destroy 继续遵循第 9 节统一生命周期，不建立逐环 timer 或 RAF。
...
- radar tail 固定使用 10 个 slot，内部协议硬上限 16。
- radar-scan round-trip 只能按 elapsed 对固定 10 个 age 区间做解析式回看；不得保存真实渲染帧、建立随帧推进的历史队列或因折返点增加 geometry。每帧最多更新 10 个既有扇形 buffer，折返点 newest-wins 的区间裁剪使用固定标量工作区，不得分配临时 Polygon、坐标数组或 Style。
- center-spread 每环固定四个填充 slot 和一个前沿 stroke slot；默认 3 环共 15 个 slot，`ringCount: 5` 时达到硬上限 25，`trailLength: 0` 也不得改变预分配拓扑。
...
- 实现 radar-scan one-way / round-trip、解析式 recent-history 与折返点 newest-wins、center-spread、角度与径向渐变尾迹、Circle 跨界连续角区间、无接缝完整环带、Sector 裁剪、弧线采样缓存、固定 slot 和 world wrap。
...
1. 每个 Spec 的默认值、边界、未知字段、非法对象、输入不变性和错误类型；radar-scan 还覆盖 `scanMode` 默认 one-way、两个合法值、非法值、direction 首程语义、默认绿色、gradient 色标格式及 color/gradient 互斥，center-spread 另覆盖默认绿色、gradient、opacity 和 trailLength 的闭区间边界。
...
9. Circle 在不同纬度和 View 投影下的米制半径转换；radar-scan 渐变 offset 端点、RGBA 插值与最终 alpha；one-way 既有行为；round-trip 在 elapsed `0`、`P / 2`、`P` 及折返点前后的起点—终点—起点路径、两种首程方向、`repeat: false` 的 `P` 完成边界、recent-history 年龄区间、newest-wins 重叠裁剪、固定 10 slot 身份和无真实帧历史；Circle 顺逆时针跨 `0/2π`（含正北起扫位置）的连续尾迹；Sector 起止边界、往复折返、跨界方向和 beam 裁剪；center-spread 四段径向渐变 offset、强度、最终 alpha、固定五 slot 拓扑、`trailLength: 0` 退化、Circle 完整环带无接缝和 Sector 环形扇面严格裁剪。
...
4. 圆形与扇面效果：pulse、radar-scan one-way / round-trip、center-spread。
5. 渐隐渐显、retain 和生命周期。
...
- 所有字段、默认值、范围、单位和完成行为。
- radar-scan 必须明确 `direction` 是首程方向、`periodMs` 是完整模式周期，并用时间轴或等价示意展示 round-trip 的 `0: 起点`、`P / 2: 终点`、`P: 起点`；不得把 `periodMs` 描述成 round-trip 单程时长。
...
- fade-out retain、DoubleArrow grow、radar-scan 默认绿色与渐变 alpha、one-way 兼容、round-trip 起点—终点—起点时间线、解析式 recent-history、折返点 newest-wins、固定 10 slot、Circle `0/2π` 无接缝、Sector radar 裁剪、center-spread 默认绿色与径向渐变 alpha、固定 25 slot 上限、线环退化及 Circle/Sector 波纹带边界和 interaction pause-and-suppress 具有明确自动化证据。
