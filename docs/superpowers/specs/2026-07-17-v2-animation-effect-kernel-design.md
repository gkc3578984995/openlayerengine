...
- 尾迹把 beamWidthDeg 均分为固定 slot。令 `i = 0` 表示扫描前沿的最新 slot，`i = slotCount - 1` 表示最旧 slot，则 `gradientOffset = slotCount === 1 ? 1 : 1 - i / (slotCount - 1)`，最新 slot 强度为 1，第 `i` 个旧 slot 的 `slotIntensity = 1 - i / slotCount`。令 `resolvedColor` 为 gradient 在 `gradientOffset` 的插值颜色，未配置 gradient 时为 color，则最终 alpha 固定为 `alpha(resolvedColor) * opacity * slotIntensity * targetOpacity`。
...
- 第 `i` 个填充 slot 使用 `gradientOffset = 1 - i / 3` 和 `slotIntensity = 1 - i / 4`。令 `resolvedColor` 为 gradient 在 `gradientOffset` 的插值颜色，未配置 gradient 时为 color，则填充最终 alpha 固定为 `alpha(resolvedColor) * opacity * slotIntensity * (1 - p) * targetOpacity`。
- 每环另有一个独立的前沿 stroke slot，颜色使用 gradient offset `1` 的插值颜色，未配置 gradient 时使用 color；其最终 alpha 固定为 `alpha(frontColor) * opacity * (1 - p) * targetOpacity`，线宽为 `strokeWidth`。`trailLength: 0` 时四个填充 slot 保持预分配但全部 inactive，只绘制该前沿 stroke，从而退化为旧式完整线环或裁剪弧；不得因退化改变 slot 拓扑。
...
- 实现 radar-scan、center-spread、角度与径向渐变尾迹、Circle 跨界连续角区间、无接缝完整环带、Sector 裁剪、弧线采样缓存、固定 slot 和 world wrap。
...
1. 每个 Spec 的默认值、边界、未知字段、非法对象、输入不变性和错误类型；radar-scan 与 center-spread 还覆盖默认绿色、gradient 色标格式及 color/gradient 互斥，center-spread 另覆盖 opacity 和 trailLength 的闭区间边界。
...
9. Circle 在不同纬度和 View 投影下的米制半径转换；radar-scan 渐变 offset 端点、RGBA 插值与最终 alpha；Circle 顺逆时针跨 `0/2π`（含正北起扫位置）的连续尾迹；Sector 起止边界、跨界方向和 beam 裁剪；center-spread 四段径向渐变 offset、强度、最终 alpha、固定五 slot 拓扑、`trailLength: 0` 退化、Circle 完整环带无接缝和 Sector 环形扇面严格裁剪。
...
- fade-out retain、DoubleArrow grow、radar-scan 默认绿色与渐变 alpha、Circle `0/2π` 无接缝、Sector radar 裁剪、center-spread 默认绿色与径向渐变 alpha、固定 25 slot 上限、线环退化及 Circle/Sector 波纹带边界和 interaction pause-and-suppress 具有明确自动化证据。
