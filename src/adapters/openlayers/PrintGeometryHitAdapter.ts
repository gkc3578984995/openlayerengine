...
export class PrintGeometryHitAdapter implements PrintGeometryHitPort {
  ...
  intersectsFootprint(elementId: string, footprint: PrintFootprint, resolution?: number): boolean {
    if (resolution !== undefined) return this.#resolvedStyleHits(elementId, resolution, footprint);
  ...
  isVisibleAt(elementId: string, resolution: number, footprint?: PrintFootprint): boolean {
    return this.#resolvedStyleHits(elementId, resolution, footprint);
  }

  #resolvedStyleHits(elementId: string, resolution: number, footprint: PrintFootprint | undefined): boolean {
    const path = this.#layerPath(elementId);
    if (path === null) return false;
    if (path !== undefined) {
    ...
    if (clip === null) return false;
    const worldWidth = this.#binding.wrapsX(elementId) ? this.#currentWorldWidth() : undefined;
    const feature = this.#binding.cloneCanonicalFeature(elementId, resolution);
    try {
