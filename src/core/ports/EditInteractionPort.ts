...
/**
 * 打开一次编辑交互所需的纯配置。Adapter 不得修改传入的控制点快照，底图选项只影响临时原生投影。
 *
 * @internal
 */
...
/**
 * Adapter 发出的纯语义编辑输入。拖动严格发出一次开始、零到多次移动，以及一次结束或取消。
 * 插入和删除是单次原子请求；校验、历史、渲染与 Element 事务仍由服务负责。
 *
 * @internal
 */
...
export interface EditInteractionHandle {
  ...
  /**
   * 停止事件并释放全部预览、监听器和原始 Element 投影抑制资源。成功后幂等；失败后可只重试未完成步骤。
   *
...
export interface EditInteractionPort {
   ...
   *
   * @param spec 目标 Element、控制点和临时底图配置。
   * @param listener 接收脱离原生对象的语义编辑输入快照。
   * @returns 完整安装的编辑交互句柄。
   * @throws 配置、目标 Element、安装或回滚无效时抛出对应错误。
   */
