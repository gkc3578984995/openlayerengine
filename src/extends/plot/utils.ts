 ...
 * @constructor
 */
export const MathDistance = (pnt1: Point, pnt2: Point): number => Math.sqrt((pnt1[0] - pnt2[0]) ** 2 + (pnt1[1] - pnt2[1]) ** 2);
 ...
 * @returns {number}
 */
export const getBinomialFactor = (n: number, index: number): number => getFactorial(n) / (getFactorial(index) * getFactorial(n - index));
