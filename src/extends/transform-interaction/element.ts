...
const elementUtil = {
  ...
  getFixedOffset(elt: HTMLElement) {
    ...
    const getOffset = (parent: HTMLElement | null): { left: number; top: number } => {
      ...
      if (this.getStyle(parent, 'position') === 'absolute' && this.getStyle(parent, 'transform') !== 'none') {
  ...
  scrollDiv(elt: HTMLElement, options: ScrollDivOptions = {}) {
    ...
    if (options.vertical && options.minibar) {
      const init = (b?: boolean) => {
        ...
        if (options.mousewheel) {
          if (scrollContainer) this.addListener(scrollContainer, ['mousewheel', 'DOMMouseScroll', 'onmousewheel'], onMouseWheel as EventListener);
    ...
    const onMouseWheel = (e: WheelEvent | { wheelDelta?: number; detail?: number }) => {
      const deltaRaw =
        'wheelDelta' in e && typeof e.wheelDelta === 'number'
          ? e.wheelDelta
          : typeof (e as { detail?: number }).detail === 'number'
            ? -((e as { detail?: number }).detail as number)
            : 0;
