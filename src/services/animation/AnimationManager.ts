...
export class AnimationManagerImpl implements AnimationManager, AnimationControlPort, AnimationPreviewPort, TransientAnimationPort {
  ...
  capturePresentationSnapshot(frame: Readonly<AnimationPrintFrame>): Readonly<AnimationPresentationSnapshot> {
    ...
    for (const [elementId, indexed] of this.#recordsByElement) {
      ...
      const prepared = this.#committedStates.get(elementId);
      if (prepared === undefined || !prepared.state.visible || group.some((record) => record.layerId !== prepared.state.layerId)) continue;
      ...
      const detached: Array<{ readonly record: ElementRecord; readonly sample: AnimationSample; readonly buffer: DetachedAnimationFrame }> = [];
      for (const record of group) {
        const sampled = this.#sampleForPrint(record, prepared, frame, capturedAt);
        detached.push({ record, sample: sampled.sample, buffer: sampled.buffer });
      ...
      for (const { record, sample, buffer } of detached) {
  ...
  #sampleForPrint(
    ...
    capturedAt: number
  ): { readonly sample: AnimationSample; readonly buffer: DetachedAnimationFrame } {
    if (record.retained) {
      if (record.sample === undefined) throw new InvalidArgumentError('Retained animation does not have a stable sample');
      return Object.freeze({ sample: record.sample, buffer: detachAnimationFrame(record.buffer) });
    }
    ...
    const output = createAnimationFrameBuffer(record.renderProfile.slots);
    ...
    return Object.freeze({ sample: sampled!, buffer: detached! });
  }
...
function assertPrintFrame(frame: Readonly<AnimationPrintFrame>): void {
  if (
    ...
    throw new InvalidArgumentError('Animation print frame must contain finite resolution, rotation, pixel ratio, and extent');
  }
}
