...
export class PrintLegendBuilder {
  ...
  #generateAutomatic(
    ...
    const presentationFrame = Object.freeze({
      center: plan.range.center,
      resolution: plan.range.resolution,
      rotation: plan.range.rotation
    });
    const indexedCandidateIds = this.#geometryHit.candidateElementIds?.(plan.range.footprint, plan.range.resolution, visibleVectorLayerIds, presentationFrame);
      ...
      .filter(
        (element) =>
          ...
          (this.#geometryHit.isVisibleAt === undefined
            ? this.#geometryHit.intersectsFootprint(element.id, plan.range.footprint, plan.range.resolution, presentationFrame)
            : this.#geometryHit.isVisibleAt(element.id, plan.range.resolution, plan.range.footprint, presentationFrame))
      )
      .sort(
        (left, right) =>
...
function replayManualLegend(
  ...
  for (const automaticItem of automatic.items) {
    ...
    items.push({
      ...
      ...(automaticItem.count === undefined ? {} : { count: automaticItem.count }),
