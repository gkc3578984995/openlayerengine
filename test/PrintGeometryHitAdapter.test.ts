...
describe('PrintGeometryHitAdapter', () => {
  ...
  it('uses the frozen print presentation frame for view-dependent canonical geometry', () => {
    const cloneCanonicalFeature = vi.fn(() => styledPoint([0, 0]));
    const adapter = new PrintGeometryHitAdapter({
      cloneCanonicalFeature,
      wrapsX: () => false,
      renderOrderOf: () => 0
    } as unknown as FeatureBinding);
    const frame = { center: [25, 30] as const, resolution: 4, rotation: Math.PI / 3 };

    expect(adapter.isVisibleAt('callout', frame.resolution, footprint, frame)).toBe(true);
    expect(cloneCanonicalFeature).toHaveBeenCalledWith('callout', frame);
  });
