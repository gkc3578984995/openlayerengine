...
describe('website Print documentation', () => {
  ...
  it('covers view-fit, box-fixed and extent-fit custom-paper specs without faking unavailable outputs', async () => {
    ...
    expect(demo).toContain("source: { mode: 'extent', extent }, scale: { mode: 'fit' }");
    expect(demo).toContain("paper: { size: 'A4', orientation: 'landscape'");
    expect(demo).toContain("paper: { size: 'A3', orientation: 'portrait'");
    expect(demo).toContain('size: { widthMm: 320, heightMm: 180 }');
    ...
    expect(demo).toContain('if (!earth.print.capabilities.pdf)');
  ...
  it('documents and visualizes the fixed page composition and explicit cleanup', async () => {
    ...
    expect(view).toContain('右侧固定“日期：”与“签发人：”两个槽位');
    expect(view).toContain('常用密级建议与自由手填');
    expect(view).toContain('图例固定在地图内左下');
