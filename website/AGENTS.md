...
- 工具返回的 use* 方法名及返回类型必须链接到工具页的规范入口锚点：工具页存在方法表时使用 `#api-methods`；概览页只展示构造器和类型定义时使用 `#api-constructor`。工具参数类型链接到其归属类型锚点。
...
- 尚未迁移到 `ApiReference` 的页面中，具有当前页锚点的方法使用蓝色可点击的 `code-fn` 样式，属性和类型使用中性可点击代码样式；不得使用不可点击的 `code-fn-inline` 代替。修改这些引用时应一并迁移到 `ApiReference`。
